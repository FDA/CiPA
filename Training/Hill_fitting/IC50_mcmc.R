...
# parameter bounds for log10(IC50) and Hill coefficient (note IC50 in nM)
lowBds<-c(-10, 0)
uppBds<-c(10, 10)
...
for(channel in channels){
    ...
    initpar<-getInitial(block ~ SSHill(conc, log10IC50, h), expdf)
    ...
    trystarts<-cbind(log10(ic50seeds),rep(0.9,length(ic50seeds)))
    trystarts<-rbind(initpar,trystarts)
    tryi<-1
    while(tryi==1 || (inherits(tryout, "try-error") && tryi<=nrow(trystarts))){
        tryout<-try({
            mf<-modFit(f=HillRes, p=trystarts[tryi,], datadf=expdf, 
    ...
    plot_IC50<-10^mf$par[1]
    plot_h<-mf$par[2]
    xvals<-c(xvals, plot_IC50/10, plot_IC50*10)
    plot_x<-10^(seq(from=min(log10(xvals)), to=max(log10(xvals)), length.out=100))
    plot_y<-SSHill(plot_x, log10(plot_IC50), plot_h)
    ...
    startp<-setNames(mf$par, c("log10IC50","h"))
    ...
    while(!converged){
        ...
        if(all(abs(gd[["z"]])<qnorm(0.975))){
            ...
            col_list[[paste0(channel,"_IC50")]]<-signif(10^MCMC$pars[,1], digits=sigdigits) # transform back to IC50
