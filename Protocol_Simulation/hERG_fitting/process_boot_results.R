...
library(optparse)
myDrugDefault = "astemizole,azimilide,bepridil,chlorpromazine,cisapride,clarithromycin,clozapine,diltiazem,disopyramide,dofetilide,domperidone,droperidol,ibutilide,loratadine,metoprolol,mexiletine,nifedipine,nitrendipine,ondansetron,pimomzide,quinidine,ranolazine,risperidone,sotalol,tamoxifen,terfenadine,vandetanib,verapamil"
...
for(drug in drugnames){
	...
	#sink("name_matching_debug_file.txt")
	#print(df_list)
	#sink()
	print(identical(names(df_list[[1]]), names(df_list[[442]]) ))
