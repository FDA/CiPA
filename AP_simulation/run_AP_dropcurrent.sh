...
#               sampling distributions), with effects on ionic currents
...
for DRUG in ${DRUGNAMES[@]}; do
    for IDX in `seq 0 99`; do
        ...
        ISTOP=$(((IDX+1)*20))
        ISTR="$ISTART"-"$ISTOP"
        Rscript AP_simulation.R -d "$DRUG" -i "$ISTR" -o > logfiles/"$DRUG"."$ISTR".drop_currents 2>&1
    done
done
